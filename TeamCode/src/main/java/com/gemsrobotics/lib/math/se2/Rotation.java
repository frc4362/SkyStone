...
public class Rotation implements IRotation2d<Rotation> {
    ...
    public Rotation getNearestPole(Rotation interval) {
        Rotation current = Rotation.identity();

        if (getRadians() < 0.0) {
            interval = interval.inverse();
        }

        final double polarWidth = abs(interval.getRadians()) / 2.0;

        while (abs(distance(current)) > polarWidth) {
            current = current.rotateBy(interval);
        }
    ...
    public static void main(String[] args) {
        System.out.println(Rotation.degrees(60).getNearestPole(Rotation.degrees(45)));
    }
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof IRotation2d)) {
            return false;
        }
