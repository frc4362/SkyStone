...
public class Translation implements ITranslation2d<Translation> {
    ...
    public boolean isWithinAngle(final Translation a, final Translation vertex, final Translation c, final boolean vertical) {
        final Translation midpoint = a.interpolate(c, 0.5);
        Translation center = (new Translation(vertex, midpoint)).normalized();
        Translation side = (new Translation(vertex, a)).normalized();
        final Translation distance = (new Translation(vertex, this)).normalized();
        ...
        return Translation.dot(distance, midpoint) > Translation.dot(side, center);
    }
